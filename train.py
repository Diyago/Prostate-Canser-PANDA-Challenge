...
class LightningCanserClassifier(pl.LightningModule):
    def __init__(self, config):
        ...
        self.config = config
        self.model = CustomSEResNeXt(config['model_params'])
    ...
    def get_loss(self, y_preds, labels):
        ...
        elif config['training']['loss'] == 'LabelSmoothingCrossEntropy':
    ...
    def optimizer_step(self, epoch, batch_idx, optimizer, optimizer_idx, second_order_closure):
        if self.trainer.global_step < config['training']['warmup_steps']:
            ...
            for pg in optimizer.param_groups:
                pg['lr'] = lr_scale * config['training']['optimizer']['kwargs']['lr']
...
# class LightningCanserClassifierLR(pl.LightningModule):
#     def __init__(self, config):
#         super(LightningCanserClassifierLR, self).__init__()
#         self.config = config
#         self.model = CustomSEResNeXt(config['model_params'])
#
#     def forward(self, x):
...
if __name__ == '__main__':
    ...
    for fold in range(config['validation']['nfolds']):
        ...
        # model = LightningCanserClassifierLR(config)
        # trainer = pl.Trainer(**config['training']['Trainer'])
        # lr_finder = trainer.lr_find(model, train_dataloader=train_loader)
        # config['training']['optimizer']['kwargs']['lr'] = lr_finder.suggestion()
        # print('new_lr is {}'.format(lr_finder.suggestion()))
